...
def find_groups(array):
    """
    Given an array of indexes, find the beginning and end value of each group of consecutive numbers.
    """
    # Identify the breakpoints where the difference between consecutive numbers is not 1
    breakpoints = np.diff(array) != 1
    # Indices of the starts of the groups
    group_starts = np.insert(array[np.where(breakpoints)[0] + 1], 0, array[0])
    # Indices of the ends of the groups
    group_ends = np.append(array[np.where(breakpoints)[0]], array[-1])
    
    # Combine the start and end of each group into tuples
    groups = list(zip(group_starts, group_ends))
    
    return groups

def select_multiple_subarrays(array, start_indices, end_indices):
    """
    Selects multiple subarrays from the given array based on lists of start and end indices.
    """
    subarrays = []
    for start, end in zip(start_indices, end_indices):
        # Select each subarray and append to the list of subarrays
        subarray = array[start:end+1]  # end_index is inclusive
...
for tem_batch_size in [1000,500,300,100,50]:
    if 1:
        if 1:
    ...
    #     for j in range(len(UAF_subcategory[i])):
            ...
            # pdb.set_trace()
            try:
                with open(save_folder + "CWE_0202_" +keywords+"iisig_data.pkl", "rb") as f: 
                        raw_iisig_data = pickle.load(f)
                with open(save_folder + "CWE_0202_" +keywords+"iisig_labels.pkl", "rb") as f: 
            ...
            label_1_indexs = np.where(raw_iisig_labels == 1)[0]
            groups = find_groups(label_1_indexs)
            
            # process the arrays and extact subarrays
            start_indices = []
            end_indices = []
            for group in groups:
                begin = group[0] - int((group[1] - group[0])/2)
                end = group[1] + int((group[1] - group[0])/2)
                if begin < 0:
                    begin = 0
                if end > len(raw_iisig_labels):
                    end = len(raw_iisig_labels)
                start_indices.append(begin)
                end_indices.append(end)
                
            iisig_labels = select_multiple_subarrays(raw_iisig_labels, start_indices, end_indices)
            iisig_data = select_multiple_subarrays(raw_iisig_data, start_indices, end_indices)

            #flat the arrays
            iisig_labels = np.concatenate(iisig_labels, axis=0)
            iisig_data = np.concatenate(iisig_data, axis=0)
            # pdb.set_trace()
            ### add distillation
            initial_percent = 0.1
            ...
            if save:
                ...
                #     print("no label =1 data for common label, skip....")
                    # continue
                save_to_file(distilled_folder + "CWE_0202_" +keywords+"_batch_"+str(batch_size),X_outlier, X_train, interesting_label, common_label)
                print("saved to file") 
                # if save :
                        # save_folder = "/data/xinqiao/akira/windowed_iisig/"
